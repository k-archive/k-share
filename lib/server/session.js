...
// { a:'bunsub', d:[id1, id2, ...] }
Session.prototype.bulkUnsubscribe = function(req, callback) {
  ...
  for (var i = 0; i < req.docs.length; i++) {
    var doc = req.docs[i],
        opstream = this._isSubscribed(doc.c, doc.id);

    if (!opstream) {
      result.push({ c: doc.c, id: doc.id, err: 'Already unsubscribed' }); 
      continue;
    }
    ...
    this._setSubscribed(doc.c, doc.id, false);
    result.push({ c: doc.c, id: doc.id }); 
  }
...
Session.prototype._checkRequest = function(req) {
  if (req.a === 'qsub' || req.a === 'qfetch' || req.a === 'qunsub') {
    ...
    // Doc-based request.
    if (typeof req.c !== 'string') return 'Invalid collection';
    if (typeof req.d !== 'string') return 'Invalid docName';
  ...
  } else if (req.a === 'bunsub') {
    // Bulk subscribe
    if (typeof req.docs !== 'object') return 'Invalid bulk unsubscribe data';
  } else {
...
Session.prototype._handleMessage = function(req, callback) {
  ...
  if (req.a === 'qsub' || req.a === 'qfetch' || req.a === 'qunsub') {
    ...
    if (req.o) {
      ...
      if (qopts.docMode && qopts.docMode !== 'fetch' && qopts.docMode !== 'sub')
  ...
  switch (req.a) {
    ...
    case 'bunsub':
      this.bulkUnsubscribe(req, function(err, response) {
        callback(err, err ? null : response);
      });
      break;
