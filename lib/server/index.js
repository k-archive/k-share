...
var ShareInstance = function(options) {
  ...
  if (options.backend) {
    ...
    // if we have .db, .redis1, .redis2 then create a redisclient for livedb
    if (options.redis1 && options.redis2) options.driver = livedb.redisDriver(options.db, options.redis1, options.redis2);
