...
Doc.prototype.destroy = function(callback) {
  var doc = this;
  // console.log('destroy', this.collection, this.name, this.ts);
  this.unsubscribe(function() {
    // Don't care if there's an error unsubscribing.
    setTimeout(function() {
      // There'll probably be nothing here seeing as how we just unsubscribed.
      for (var type in doc._subscribeCallbacks) {
        for (var i = 0; i < doc._subscribeCallbacks[type].length; i++) {
          doc._subscribeCallbacks[type][i]('Document destroyed');
        }

        doc._subscribeCallbacks[type].length = 0;
      }
    }, 0);
...
Doc.prototype._onMessage = function(msg) {
  // console.log('_onMessage', this.collection, this.ts);
...
Doc.prototype._onConnectionStateChanged = function(state, reason) {
  if (state === 'connecting') {
    ...
    this.subscribed = false;
    this._setWantSubscribe('subscribe');
    this.connection.flush(this);
...
Doc.prototype._clearAction = function(expectedAction) {
  // console.trace('_clearAction', this.collection, this.action, expectedAction);
  // console.log('_clearAction', this.collection, this.name, this.ts);
...
Doc.prototype.flush = function() {
  // console.log('flush', this.collection, this.name, 'return = ' + (!this.connection.canSend || this.action), this.ts);
  ...
  else this._handlePendingSubscribe();
};


// ****** Subscribing, unsubscribing and fetching

Doc.prototype._hasPendingSubscribe = function(which) {
  for (var i = 0; i < this.wantSubscribe.length; i++) {
    if (this.wantSubscribe[i] === which) {
      return true;
    }
  }
};
...
Doc.prototype._clearSubscribe = function(which) {
  ...
  // console.log('_clearSubscribe', this.collection, which, this.ts);

  if (which) {
    this._subscribeCallbacks[which].length = 0;
  }
  else {
    this._subscribeCallbacks = {
      fetch: [],
  ...
  do {
    removed = false;
    i = this.wantSubscribe.indexOf(which);

    if (i !== -1) {
      this.wantSubscribe.splice(i, 1);
      removed = true;
    }
  } while (removed);
};

Doc.prototype._handlePendingSubscribe = function() {
  // console.log('_handlePendingSubscribe', this.collection, this.name, this.ts)
  if (this.wantSubscribe.length) {
    var act = this.wantSubscribe.shift();

    if (this.subscribed) {
      // console.log(' ..... ', this.collection, this.name, this.ts, 'action = unsubscribe')
      if (act === 'unsubscribe') {
        this.action = act;
        this._send({a:'unsub'});
      }
      ...
      switch (act) {
        case 'fetch':
          // console.log(' ..... ', this.collection, this.name, 'action = fetch', this.ts)
          this.action = act;
          this._send(this.state === 'ready' ? {a:'fetch', v:this.version} : {a:'fetch'});
          break;
        case 'subscribe':
          // console.log(' ..... ', this.collection, this.name, 'action = subscribe', this.ts)
          this.action = act;
...
Doc.prototype._setWantSubscribe = function(value, callback) {
  ...
  if (
      (value === 'unsubscribe' && !this.subscribed) ||
      ((value === 'fetch' || value === 'subscribe') && this.subscribed)
      ) {
    if (callback) callback();
    return;
  }
  ...
  if (value === 'subscribe') {
    // console.log('_setWantSubscribe subscribe', this.collection, this.name, this.ts);
    // if we want to subscribe but are already set to want fetch, remove the fetch because it's 'weaker'
    if (this._hasPendingSubscribe('fetch')) {
      this._clearSubscribe('fetch');
    }

    this.wantSubscribe.push(value);
  }
  else if (value === 'fetch') {
    // console.log('_setWantSubscribe fetch', this.collection, this.name, this.ts);
    // If we already want to subscribe, don't weaken it to a fetch.
    if (this._hasPendingSubscribe('subscribe')) {
      return;
    }
  ...
  // ubsubscribe
    ...
    // console.log('_setWantSubscribe unsubscribe', this.collection, this.name, this.ts);
    this.wantSubscribe.push(value);
  }
...
Doc.prototype._action = function(action, callback) {
  // console.log('_action', action, this.collection, this.name, this.ts);
  var ok = this._setWantSubscribe(action, callback);

  if (ok) {
    if (callback) this._subscribeCallbacks[action].push(callback);
    this.connection.flush(this);
  }
};

Doc.prototype.startAction = function(action, callback) {
  // console.log('startAction', action, this.collection, this.name, this.ts);
  this.action = action;
  if (callback) this._subscribeCallbacks[action].push(callback);
};
...
Doc.prototype._finish = function(value, error) {
  // console.log('_finish', this.collection, this.action, value, this.ts);
  if (value !== 'fetch') {
    this.subscribed = value === 'subscribe';
    this.emit(value);
  }

  this._finishSub(value, error);
  this._clearSubscribe(value);
  this._clearAction(value);
}

Doc.prototype._finishSub = function(value, error) {
  // console.log('_finishSub', this.collection, value, error, this.action, this.ts);
  if (value) {
    if (this.action === value) {
      for (var i = 0; i < this._subscribeCallbacks[value].length; i++) {
        this._subscribeCallbacks[value][i](error);
      }
      this._subscribeCallbacks[value].length = 0;
    }
    else {
      console.warn('_finishSub: expected ', this.action,' got', value);
    }
  }
};

Doc.prototype._finishQuerySubscribe = function(version) {
  // console.log('_finishQuerySubscribe', this.collection, this.name, this.ts);
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  ...
  var _this = this;
  setTimeout((function() { _this.connection.flush(_this); }), 0);
};
