...
var Doc = exports.Doc = function(connection, collection, name) {
  ...
  // NOTE: this has been changed to a FIFO stack
  this.actions = [];
...
Doc.prototype.destroy = function(callback) {
  var doc = this;
  // console.log('destroy', this.collection, this.name);

  if (this.actions.length) {
    // console.log(this.actions.length, this.actions);
  ...
  this.unsubscribe(function() {
    ...
    setTimeout(function() {
      ...
      // for (var type in doc.callbacks) {
      //   if (doc.callbacks[type].length) {
      //     // console.log('callback left', type, doc.callbacks[type].length, doc.collection, doc.name);
      //     // console.log(doc.callbacks);
      //   }
      // }

      for (var type in doc.callbacks) {
        for (var i = 0; i < doc.callbacks[type].length; i++) {
          doc.callbacks[type][i]('Trying to execute callbacks but document was already destroyed');
        }

        doc.callbacks[type].length = 0;
      }
    }, 0);
...
Doc.prototype.ingestData = function(data) {
  if (this.state) {
    if (typeof console !== "undefined") {
      console.warn('Ignoring attempt to ingest data in state', this.state, 'collection = ', this.collection, this.name);
      // console.trace();
    }
    return;
  }
...
Doc.prototype._onMessage = function(msg) {
  // console.log('_onMessage', this.collection, msg.a);
  ...
  switch (msg.a) {
    case 'fetch':
      ...
      this._finish('fetch', msg.error);
      this.connection.flushCollection(this.collection);
      break;
    ...
    case 'unsub':
      // Unsubscribe reply
      this._finish('unsubscribe', msg.error);
      this.connection.flushCollection(this.collection);
      break;
...
Doc.prototype._onConnectionStateChanged = function(state, reason) {
  if (state === 'connecting') {
    ...
    this.action = null;
    wereSubscribed = this.subscribed;
    this.subscribed = false;
    this._addAction('subscribe');
    this.connection.flushCollection(this.collection);
    if (wereSubscribed) this.emit('unsubscribe', { c: this.collection, id: this.name });
  }
};

// ****** Dealing with actions

Doc.prototype._clearCurrentAction = function(expectedAction) {
  // console.log('_clearCurrentAction', this.collection, this.name);

  if (expectedAction && this.action !== expectedAction) {
...
Doc.prototype.flush = function() {
  // console.log('flush', this.collection, this.name, 'return = ' + (!this.connection.canSend || this.action));
  ...
  else this._handlePendingActions();
};


// ****** Subscribing, unsubscribing and fetching

Doc.prototype._hasPendingAction = function(which) {
  for (var i = 0; i < this.actions.length; i++) {
    if (this.actions[i] === which) {
      return true;
    }
  }
};

Doc.prototype._clearActions = function(which) {
  var i, removed;

  // console.log('_clearActions', this.collection, which);

  if (which) {
    // console.log('clearing callbacks '+which);
    this.callbacks[which].length = 0;
  }
  else {
...
Doc.prototype._handlePendingActions = function() {
  // console.log('_handlePendingActions', this.collection, this.name, this.actions.length)
  if (!this.action && this.actions.length) {
    var act = this.actions.shift();

    if (this.subscribed) {
      // console.log(' ... ', this.collection, this.name, 'this.subscribed = '+this.subscribed, 'act = ' + act);
      if (act === 'unsubscribe') {
        this.action = act;
        this._send({a:'unsub'});
      }
      else {
        // clear subscribes because we are subscribed and we want to subscribe mor
        this._finishAction(act);
      }
    } else {
      switch (act) {
        case 'fetch':
          // console.log(' ..... ', this.collection, this.name, 'action = fetch')
          this.action = act;
          this._send(this.state === 'ready' ? {a:'fetch', v:this.version} : {a:'fetch'});
          break;
        case 'subscribe':
          // console.log(' ..... ', this.collection, this.name, 'action = subscribe')
          this.action = act;
...
Doc.prototype._handleSubscribe = function(err, data) {
  // console.trace('_handleSubscribe', data);
  if (err && console) console.error("_handleSubscribe Could not subscribe: " + err, this.collection, this.name);
  if (err && err !== 'Already subscribed') {
    this.emit('error', err);
    // There's probably a reason we couldn't subscribe. Don't retry.
    this._clearActions()
    this._clearCurrentAction(null);
  } else {
    if (data) this.ingestData(data);
    this._finish('subscribe', null);
  }
...
// Value is 'subscribe', 'unsubscribe' or 'fetch'.
Doc.prototype._addAction = function(value, callback) {
  // console.log('_addAction', this.collection, this.name, ', subscribed = '+this.subscribed, ', current action = '+this.action, ', adding '+value);
  // console.log('this.actions', this.actions);

  // if we have some actions in the middle, remove them
  // after all, things won't change if we end up where we are now
  if (this.actions.length && this.action === value) {
    // console.log('clearing this.actions')
    this._clearActions();
  }

  // already at the correct state?
  // skip this optimization if w ehave some pending actions as they can change things
  if (!this.actions.length) {
    if (this.action === value ||
        (value === 'unsubscribe' && !this.subscribed && !this.action) ||
        ((value === 'fetch' || value === 'subscribe') && this.subscribed && !this.action)
        ) {
      // console.log('... returning immediately, this.action = ', this.action)
      if (callback) callback();
      return;
    }
  }
  ...
  if (value === 'subscribe') {
    // if we want to subscribe but are already set to want fetch, remove the fetch because it's 'weaker'
    if (this._hasPendingAction('fetch')) {
      this._clearActions('fetch');
    }

    this.actions.push(value);
  }
  else if (value === 'fetch') {
    // If we already want to subscribe, don't weaken it to a fetch.
    if (this._hasPendingAction('subscribe')) {
      if (callback) callback();
      return;
    }
...
Doc.prototype.subscribe = function(callback) {
  this._action('subscribe', callback);
};

// Unsubscribe. The data will stay around in local memory, but we'll stop
// receiving updates.
Doc.prototype.unsubscribe = function(callback) {
  this._action('unsubscribe', callback);
};

// Call to request fresh data from the server.
Doc.prototype.fetch = function(callback) {
  this._action('fetch', callback);
};

Doc.prototype._action = function(action, callback) {
  // console.trace('_action', action, this.collection, this.name);
  // console.log('_action callbacks', this.callbacks);
  var ok = this._addAction(action, callback);

  if (ok) {
    // console.log('ok - adding a callback to ', action, this.collection, this.name);
    if (callback) this.callbacks[action].push(callback);
    this.connection.flushCollection(this.collection);
  }
};

Doc.prototype.startAction = function(action, callback) {
  // console.log('startAction', action, this.collection, this.name);
  this.action = action;
  if (callback) this.callbacks[action].push(callback);
};

// Called when our subscribe, fetch or unsubscribe messages are acknowledged.
Doc.prototype._finish = function(value, error) {
  // console.log('_finish', this.collection, this.action, value);
  this.subscribed = value === 'subscribe' || value === 'fetch';

  this._finishAction(value, error);
  this._clearActions(value);
  this._clearCurrentAction(value);

  this.emit(value, { c: this.collection, id: this.name });
}

Doc.prototype._finishAction = function(value, error) {
  // console.log('_finishAction', this.collection, value, error, this.action);
  if (value) {

    if (this.action !== value) {
      console.warn('_finishAction: expected ', this.action,' got', value);
    }

    for (var i = 0; i < this.callbacks[value].length; i++) {
      this.callbacks[value][i](error);
    }

    // console.log('2) clearing callbacks '+value);
    this.callbacks[value].length = 0;
  }
};

Doc.prototype._finishQuerySubscribe = function(version) {
  // console.trace('_finishQuerySubscribe', this.collection, this.name);
...
Doc.prototype._sendOpData = function() {
  ...
  // console.log('set action', this.collection, this.action, 'submit');
  this.action = 'submit';
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  // console.trace('Doc.prototype._submitOpData', this.collection, this.state, clone(opData));
  ...
  if (!this.state) {
    // console.log('set state floating', this.collection);
    this.state = 'floating';
  }
  ...
  var _this = this;
  setTimeout((function() { _this.connection.flushCollection(_this.collection); }), 0);
};
...
Doc.prototype.create = function(type, data, context, callback) {
  ...
  if (this.type) {
    if (callback) callback('Doc.js: document already exists (' + this.collection+'.' + this.name + ')', this._opErrorContext(op));
    return 
  }
...
Doc.prototype._clearInflightOp = function(error) {
  ...
  this.inflightData = null;
  this._clearCurrentAction('submit');
  this.connection.flushCollection(this.collection);
