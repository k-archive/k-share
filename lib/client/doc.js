  ...
  switch (msg.a) {
    ...
    case 'op':
      ...
      if (msg.v > this.version) {
        ...
        // TODO: unsubscribe + resubscribe to get the data
        break;
      }
...
Doc.prototype._handlePendingActions = function() {
  // console.log('_handlePendingActions', this.collection, this.name, this.actions.length, 'this.subscribed = '+this.subscribed)
    ...
    switch (act) {
      case 'unsubscribe':
        if (this.subscribed) {
          ...
          // clear subscribes because we are subscribed and we want to subscribe more
...
Doc.prototype._finishAction = function(value, error) {
  ...
  if (value) {
    ...
    if (this.action !== value) {
      console.warn('_finishAction: expected ', this.action, ', got', value, "(" + this.collection + "." + this.name + ")");
    }
