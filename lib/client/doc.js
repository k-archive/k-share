...
Doc.prototype.bug = function(s, trace) {
    console.log(s, this);
}

Doc.prototype.bug = function(s, trace) {
  if (trace && typeof trace !== 'number') {
    console.log(s+' ('+ this.collection+'.'+ this.name+'), subscribed = '+this.subscribed+', current action = '+this.action, ',', trace, ', this.actions =', clone(this.actions), ', this.callbacks = ', clone(this.callbacks));
  }
  else {
    console.log(s+' ('+ this.collection+'.'+ this.name+'), subscribed = '+this.subscribed+', current action = '+this.action, ', this.actions =', clone(this.actions), ', this.callbacks = ', clone(this.callbacks));
  }
...
Doc.prototype.destroy = function(callback) {
  ...
  this.unsubscribe(function() {
    ...
    // We should probably check if there are any callbacks left
    // that haven't been fired.

    // Also, should we move the removeContexts() call
    // to accompany the destroyDoc() call?
...
Doc.prototype.hasPending = function() {
  return this.action != null || this.inflightData != null || !!this.pendingData.length || !!this.actions.length;
};
...
Doc.prototype._onMessage = function(msg) {
  ...
  switch (msg.a) {
    ...
    case 'op':
      ...
      if (this.inflightData) {
        this.xf(this.inflightData, msg);
      }

      for (var i = 0; i < this.pendingData.length; i++) {
        this.xf(this.pendingData[i], msg);
      }
...
Doc.prototype._onConnectionStateChanged = function() {
  if (this.connection.canSend) {
    ...
    this.subscribed = false;
    if (wereSubscribed) {
      this.emit('unsubscribe', { c: this.collection, id: this.name });
...
Doc.prototype.flush = function() {
  ...
  // What is this, why is this here, should we remove this?
  var opData;
  ...
  // Move this to _handlePendingActions()?
  // "paused" should probably be checked there as well.
...
Doc.prototype._clearActions = function(which) {
  ...
  // this.bug('_clearActions ' + which, 1);
  ...
  // this.bug('removed ' + which);
};

Doc.prototype._handlePendingActions = function() {
  // this.bug('Doc.prototype._handlePendingActions')
  if (!this.action && this.actions.length) {
    var act = this.actions.shift();

    switch (act) {
      case 'unsubscribe':
        if (this.subscribed && this.fromQuery) {
          this.action = act;
          this._finish(act);
        }
        else if (this.subscribed && !this.fromQuery) {
          this.action = act;
          this._send({a:'unsub'});
        }
        else {
...
Doc.prototype._finishAction = function(value, error) {
  ...
  // this.bug('removed '+value);
};

Doc.prototype._finishQuerySubscribe = function(version) {
  // this.bug('_finishQuerySubscribe');
  ...
  if (version > this.version) {
    console.log('version > this.version', version, this.version)
    console.log('subscribing (' + this.collection + '.' + this.name + ')..')
...
Doc.prototype.xf = function(client, server) {
  ...
  if (client.create) {
    throw new Error('Invalid state. This is a bug. ' + this.collection + ' ' + this.name);
  }
...
Doc.prototype.sendFetch = function() {
  var msg = {a: 'fetch', c: this.collection, d: this.name};
  if (this.version != null) msg.v = this.version;
  this.connection.send(msg);
};

Doc.prototype.sendUnsubscribe = function() {
  var msg = {a: 'unsub', c: this.collection, d: this.name};
  this.connection.send(msg);
};

Doc.prototype.sendOp = function(data) {
  var msg = {
    a: 'op',
    c: this.collection,
    d: this.name,
...
Doc.prototype._submitOpData = function(opData, context, callback) {
  // console.log('_submitOpData', JSON.stringify(opData), 'v=', this.version);
...
Doc.prototype._tryRollback = function(opData) {
  ...
  if (opData.create) {
    ...
    for (var i = 0; i < this.pendingData.length; i++) {
      this.xf(this.pendingData[i], opData);
    }
