...
Doc.prototype._matchSubscribed = function(which) {
  return (this.wantSubscribe.length && this.wantSubscribe[0] === which) || (!which && !this.wantSubscribe.length);
};
...
Doc.prototype._setWantSubscribe = function(value, callback, err) {
  if (this._matchSubscribed(this.subscribed) &&
      (this.subscribed === value || value === 'fetch' && this.subscribed)) {
    if (callback) callback(err);
    return;
  }
  
  if (!this._matchSubscribed(value)) {
