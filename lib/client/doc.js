...
Doc.prototype._onMessage = function(msg) {
  ...
  switch (msg.a) {
    ...
    case 'unsub':
      ...
      this._finishSub('unsubscribe', msg.error);
...
Doc.prototype._handlePendingSubscribe = function() {
  if (this.wantSubscribe.length) {
    var wantSubscribe = this.wantSubscribe.shift();

    if (this.subscribed && !wantSubscribe) {
      this.action = 'unsubscribe';
      this._send({a:'unsub'});
    } else if (!this.subscribed && wantSubscribe === 'fetch') {
      this.action = 'fetch';
      this._send(this.state === 'ready' ? {a:'fetch', v:this.version} : {a:'fetch'});
    } else if (!this.subscribed && wantSubscribe) {
...
Doc.prototype._handleSubscribe = function(err, data) {
  if (err && err !== 'Already subscribed') {
    ...
    this._clearSubscribe(false)
  } else {
    if (data) this.ingestData(data);
    this.subscribed = true;
    this.emit('subscribe');
    this._finishSub('subscribe');
  }
...
Doc.prototype._setWantSubscribe = function(value, callback, err) {
  ...
  if (value !== 'fetch' || !this._hasPendingSubscribe(true)) {
