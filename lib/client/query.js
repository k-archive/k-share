...
var Query = exports.Query = function(type, connection, id, collection, query, options, callback) {
  ...
  // has tis query been executed already?
  this.executed = false;
...
Query.prototype._execute = function() {
  if (!this.connection.canSend || this.executed) return;

  if (this.connection.isConflictingAction(this.collection, this.type)) {
    return;
  }
  ...
  if (this.docMode) {
    ...
    // NOTE: we can actually get these from the main collection as it holds the sae information"
    // and possibly more. CHECK THIS.
    var existing = this.connection.getExistingCollection(this.collection);
...
Query.prototype._dataToDocs = function(data) {
  ...
  // we need to sort the array if this was an id query
  // the result needs to be in the same order as the id array
  if (this.query && this.query._id && this.query._id.$in) {
    var results2 = [];

    for (i = 0; i < this.query._id.$in.length; i++) {
      var r = this.query._id.$in[i];

      for (var j = 0; j < results.length; j++) {
        if (results[j].name === r) {
          results2.push(results[j]);
          continue;
        }
      }
    }
