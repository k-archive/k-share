...
var Connection = exports.Connection = function (socket) {
  ...
  // bulk unsub callbacks
  this.bunsubCallbacks = {};
    ...
    case 'bunsub':
      if (msg.docs && msg.docs.length) {
        for (var i = 0; i < msg.docs.length; i++) {
          var doc = this.get(msg.docs[i].c, msg.docs[i].id);

          if (doc) {
            doc._finish('unsubscribe');
          }
        }
      }

      if (msg.id && this.bunsubCallbacks && this.bunsubCallbacks[msg.id]) {
        this.bunsubCallbacks[msg.id]();
        this.bunsubCallbacks[msg.id].length = 0;
      }
      break;
...
// bajillion messages over browserchannel. During reconnect we'll aggregate,
// similar to sendOp.
Connection.prototype.bulkUnsubscribe = function(docs, cb) {
  var doc;

  for (var i = 0; i < docs.length; i++) {
    doc = this.get(docs[i].c, docs[i].id);
    doc.startAction('unsubscribe', docs[i].cb);
  }

  var msg = { a: 'bunsub', id: Date.now(), docs: docs };
  this.bunsubCallbacks[msg.id] = cb;
  this.send(msg);
};
...
Connection.prototype.flush = function(doc) {
  var collection = this.collections[doc.collection], key, doc2, query;

  if (collection) {
    for (key in collection) {
      doc2 = collection[key];
      if (doc.collection === doc2.collection) {
        doc2.flush();
      }
    }
  }
  ...
  for (key in this.queries) {
    ...
    if (doc.collection === query.collection) {
      query._execute();
    }
  }
}
